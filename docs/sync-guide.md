...
3. checkout 源仓 `KG-AT-HOME/knowledge-agent`
4. checkout 源仓 `KG-AT-HOME/qa-agent`
5. 用 Actions 中的源仓路径覆盖 `SOURCE_ROOT_NL2CYPHER`、`SOURCE_KNOWLEDGE` 和 `SOURCE_QA`
...
GitHub Actions 也会通过 `SOURCE_KNOWLEDGE` 和 `SOURCE_QA` 覆盖两个外部 agent 的默认本地路径。
...
`knowledge-agent` 和 `qa-agent` 也可以分别用 `sync_knowledge.sh` 和 `sync_qa.sh` 单独同步。
