...
- Prompt evidence from IssueTicket：生成证据快照来自 testing-agent 的失败事件快照，而不是 repair-agent 回查 cypher-generator-agent。
...
`IssueTicket` 是 testing-agent 提交给 repair-agent 的失败问题单。它只在 testing-agent 对某次成功 submission 或可评分的 `CgaGenerationNonSuccessReport(generation_failed)` 的最终评测结论不是 `pass` 时生成；`clarification_required` 和 `service_failed` 只由 testing-agent 落盘，不进入 repair-agent。
...
| `generation_evidence` | cypher-generator-agent 生成过程证据，由 testing-agent 从 `GeneratedCypherSubmissionRequest` 或 `CgaGenerationNonSuccessReport(generation_failed)` 中复制到问题单 |

它的意义是：把一次失败事件中 repair-agent 需要的事实都固定下来。repair-agent 不再为了补齐 prompt 或生成证据回头查询 cypher-generator-agent。

### 1.3 `GenerationEvidence`

`GenerationEvidence` 表示 cypher-generator-agent 生成阶段留下的过程证据。它的来源是 cypher-generator-agent 提交给 testing-agent 的 `GeneratedCypherSubmissionRequest` 或可评分的 `CgaGenerationNonSuccessReport(generation_failed)`。testing-agent 不重新生成这些字段，只在接收时保存，并在生成 `IssueTicket` 时复制到 `generation_evidence`。
...
| `input_prompt_snapshot` | cypher-generator-agent 本轮生成证据快照，当前主契约为 `cga_trace_v2` |
...
`DiagnosisContext` 是 repair-agent 内部传给 LLM 诊断客户端的结构化上下文。它不是外部 API 契约，而是把 `IssueTicket` 和生成证据快照压缩成更适合根因分析的材料。
...
| `prompt_evidence` | 压缩后的生成证据文本；名称沿用历史实现，内容可能是语义 trace 或 LLM fallback prompt |
...
repair-agent 同时保留两类生成证据文本：

| 字段 | 来源 | 作用 |
| --- | --- | --- |
| `prompt_evidence` | `generation_evidence.input_prompt_snapshot` | 压缩后的生成证据文本，保证中文和结构化语义 trace 不会被抽空 |
| `relevant_prompt_fragments` | 从生成证据文本中按关键词抽取的片段 | 辅助判断证据中是否已有 system rule、business knowledge、few-shot 或 repair 信息 |
...
| `id` | QA 样本主键 |
| `prompt_snapshot` | repair-agent 实际用于诊断的生成证据快照，来源于 IssueTicket；字段名沿用历史实现 |
...
| 提取生成证据 | `IssueTicket.generation_evidence` | evidence snapshot | 使用 testing-agent 固化的失败事件快照作为事实源 |
| 构造上下文 | `IssueTicket` 与生成证据 | `DiagnosisContext` | 将失败事实整理为 LLM 可消费材料 |
...
repair-agent 当前不向 cypher-generator-agent 拉取 prompt 或语义 trace。
...
repair-agent 只读取 `IssueTicket.generation_evidence.input_prompt_snapshot`。`generation_evidence.input_prompt_snapshot` 缺失时，请求会被输入模型拒绝；字段存在但为空字符串时，本轮 `prompt_evidence` 为空。repair-agent 不伪造 prompt 或语义 trace，不回查 cypher-generator-agent。
...
| `prompt_evidence` | 从正式生成证据快照压缩、去重后保留的完整证据文本 |
| `generation_evidence` | `IssueTicket.generation_evidence` 的生成证据摘要 |
| `relevant_prompt_fragments` | 从生成证据快照中抽取 system、business、few-shot、recent repair 片段，只作辅助证据 |
...
5. `generation_evidence`：复制 testing-agent 的 generation evidence，并把其中的 `input_prompt_snapshot` 替换为最多 600 字符的压缩证据文本，避免在诊断上下文中重复塞入完整 trace。
6. `relevant_prompt_fragments`：用简单关键词从生成证据快照抽取辅助片段：
...
```json
{
  ...
  "prompt_evidence": "压缩后的生成证据文本",
  "generation_evidence": {
    "generation_run_id": "gen-001",
    "attempt_no": 2,
    "input_prompt_snapshot": "压缩后的生成证据文本",
    "generation_status": "generated",
    "failure_reason": null
  },
...
`sql_pair.actual_cypher` 来自 `IssueTicket.actual.generated_cypher`。对成功 submission，它是本次生成的 query；对 `generation_failed`，它是 testing-agent 从 `parsed_cypher` 派生出的候选文本；没有可解析候选时为空字符串。
...
5. 最后看 DiagnosisContext.prompt_evidence：判断 expected 需要的 schema/path/business term/输出约束在生成证据快照里是否存在，以及 actual 是否正确使用了这些知识。
字段说明：
...
- DiagnosisContext.prompt_evidence 是 cypher-generator-agent 当次提交的生成证据快照，已压缩用于诊断；当前语义流水线主链路中它通常是语义 trace，不一定是完整 LLM prompt。
...
- 生成证据文本会先按行去重；过长时保留头尾，中间用 `...[prompt truncated]...` 标记。
...
repair 分析记录在此基础上额外保留 `prompt_snapshot`、`confidence`、`rationale`、LLM prompt snapshots 和 `diagnosis_context_summary` 等追溯字段。其中 `prompt_snapshot` 字段名沿用历史实现，保存的是来自 IssueTicket 的生成证据快照。
...
- repair-agent 不从 cypher-generator-agent 拉 prompt 或语义 trace。
- repair-agent 只使用 `IssueTicket.generation_evidence.input_prompt_snapshot`。
- 中文或结构化语义 trace 能进入 `prompt_evidence`。
...
repair 分析记录中的 `prompt_snapshot` 保存的是 repair-agent 实际用于诊断的生成证据快照。字段名沿用历史实现；当前语义流水线主链路中它通常是语义 trace，不一定是完整 LLM prompt。
...
`generation_evidence.input_prompt_snapshot` 是 `IssueTicket` 的必填字段；字段缺失时会被输入模型拒绝。字段存在但为空字符串时，repair-agent 不会伪造 prompt 或语义 trace；诊断上下文中的 prompt evidence 为空，LLM 仍基于其他失败证据进行诊断。
