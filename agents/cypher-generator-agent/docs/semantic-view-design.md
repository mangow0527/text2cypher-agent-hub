...
提示词必须是小型选择题，不能把全量语义视图灌入提示词。运行时只传入有限候选卡片，每张卡片来自候选生成、语义补全和候选过滤排序后的合法候选。
...
- 适用表达：目的网元、目标网元、到达网元、终点网元、目的端网元
- 不适用表达：源网元、起点网元、来源网元、经过网元、穿过网元、路径上的网元
- 命中证据：对应网元 / 目的网元 / LLM 有限候选消歧前的候选理由
```

字段含义：

```text
1. path_semantic：候选指向的语义视图路径语义 ID，必须来自 network_graph_semantic_view.yaml。
2. 业务含义：给模型看的中文解释，说明该候选在业务上代表什么。
3. relationships：该路径语义展开后的单跳关系链，用于帮助模型理解候选差异。
4. 适用表达：自然语言中明确指向该候选的正向信号。
5. 不适用表达：自然语言中出现时不应选择该候选的反向信号。
6. 命中证据：该候选进入消歧的自然语言证据或规则解释。
```
...
1. 只有当用户问题中出现候选的正向信号，且没有出现该候选的不适用表达时，才输出 `decision=accept`，并返回候选的 `path_semantic`。
2. 如果用户只说“对应、相关、关联、涉及、有哪些”等泛化表达，没有出现源端、目的端、到达、经过、穿过、路径上等明确消歧词，必须输出 `decision=clarify`，不要猜。
3. 如果多个候选都合理，且用户问题没有明显倾向，输出 `decision=clarify`，并给出中文澄清问题。
4. 如果候选都不符合用户问题，输出 `decision=reject`。
...
```text
用户问题：查询服务使用隧道到达的目的网元名称
候选：包含 service.tunnel_source、service.tunnel_destination、service.tunnel_path
判断：问题中出现“到达”“目的网元”，应选择 service.tunnel_destination
```
...
```jsonc
{
  ...
  "confidence": 0.9,
  "reason": "问题明确出现“到达的目的网元”，符合目的网元候选。",
  "clarification_question": null
}
```
...
用户问题：查询服务对应的网元
候选：包含 service.tunnel_source、service.tunnel_destination、service.tunnel_path
判断：问题只说“对应网元”，没有源端、目的端、经过等消歧词，不能猜测
```

```jsonc
{
  "decision": "clarify",
  "selected_path_semantic": null,
  "confidence": 0.0,
  "reason": "“对应网元”可以指源网元、目的网元或路径经过的网元，当前问题没有明确倾向。",
  "clarification_question": "你说的“对应网元”是指源网元、目的网元，还是路径经过的网元？"
}
```
...
```jsonc
{
  // accept 表示模型在有限候选中选择了一个候选。
  // clarify 表示有限候选仍无法可靠区分，需要澄清反问。
  // reject 表示候选都不符合用户问题。
  "decision": "accept",

  // 被选择的路径语义 ID，必须来自本次提示词中的候选卡片。
  "selected_path_semantic": "service.tunnel_destination",

  // 模型对本次选择的自评置信度，只用于诊断和运行中心展示，不直接替代工程校验。
  "confidence": 0.82,

  // 中文选择理由，用于运行中心展示和后续排查。
  "reason": "问题中的“对应网元”在当前候选中更符合服务经隧道到达的目的网元。",

  // 仅当 decision=clarify 时填写；accept/reject 时为空或省略。
  "clarification_question": null
}
```
...
```text
1. decision 必须是 accept、clarify 或 reject
2. accept 时 selected_path_semantic 必须来自本次候选卡片
3. 不能返回候选之外的语义对象
4. 不能包含 Cypher、schema 片段或自然语言解释正文
5. 选择结果必须再次通过语义视图和真实 schema 校验
6. 校验通过后，重新构造 SemanticMatchResult，再交给 planner
```

后处理规则：

```text
1. decision=accept 且 selected_path_semantic 合法：
   - 将候选路径收敛为单一路径语义
   - 重新执行 returns、metrics、order_by、limit 等语义补全
   - 输出 accepted=true 的 SemanticMatchResult

2. decision=clarify：
   - 输出 needs_clarification=true
   - clarification_question 使用 LLM 返回的中文问题，或使用工程默认澄清问题
   - 不进入 planner

3. decision=reject 或输出不合法：
   - 保留原始候选和 LLM 调用记录
   - 不采纳模型选择
   - 根据原始歧义结果进入澄清或拒绝
```
