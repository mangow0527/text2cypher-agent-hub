...
`GeneratedCypherSubmissionRequest` 表示 cypher-generator-agent 提交给 testing-agent 的一次可评测 Cypher 生成物和生成证据。它不是 testing-agent 单方面定义的内部对象，而是 cypher-generator-agent 与 testing-agent 之间的跨服务契约，必须与 `services/cypher_generator_agent/app/api/models.py` 中的成功提交契约保持一致。
