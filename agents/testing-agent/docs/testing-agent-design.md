...
### 1.3 cypher-generator-agent -> testing-agent 成功提交契约：`GeneratedCypherSubmissionRequest`

`GeneratedCypherSubmissionRequest` 表示 cypher-generator-agent 提交给 testing-agent 的一次可评测 Cypher 生成物和生成证据。它不是 testing-agent 单方面定义的内部对象，而是 cypher-generator-agent 与 testing-agent 之间的跨服务契约，必须与 `services/cypher_generator_agent/docs/cypher-generator-agent-design.md` 中的成功提交契约保持一致。
...
| `generation_status` | 成功 submission 固定为 `generated`；CGA 对调用方的本轮服务输出可以是 `submitted_to_testing` |
| `generated_cypher` | cypher-generator-agent 认为可提交评测的 Cypher |
| `input_prompt_snapshot` | 生成证据快照。当前主契约为 `cga_trace_v2`，保存 intent、semantic view matching、LogicalQueryPlan、schema path planning、RAG 知识选择、renderer、Cypher fallback LLM、preflight、clarification 和 delivery 证据；它不参与主评测打分 |
...
### 1.3.1 cypher-generator-agent -> testing-agent 非成功输出契约：`CgaGenerationNonSuccessReport`

当 cypher-generator-agent 没有得到可提交评测的 `generated_cypher` 时，也需要把本轮非成功输出提交给 testing-agent。这样 testing-agent 才能成为运行中心回放的统一数据源，而不是让运行中心分别读取多个服务的临时日志。

该契约独立于成功 submission，命名为 `CgaGenerationNonSuccessReport`。持久化目录可以继续使用 `generation_failures/`，但该目录保存的是 CGA 非成功输出报告，包含 `generation_failed`、`clarification_required` 和 `service_failed`。
...
| `gate_passed` | 仅 `generation_failed` 可使用；表示最后一次生成侧门禁是否通过，通常为 `false` |
...
```jsonc
{
  ...
  "input_prompt_snapshot": "{ cga_trace_v2 JSON string }",
  "failure_reason": "logical_plan_mismatch",
  "parsed_cypher": null,
  "gate_passed": false
}
```
...
- 澄清请求：`clarification_required` 表示本轮单轮问题缺少必要信息或存在业务歧义。testing-agent 保存澄清报告，供运行中心展示澄清问题和选项；不构造正式评测 attempt，不进入 grammar、TuGraph execution、strict compare、semantic review 或 repair-agent。
- 生成失败：`generation_failed` 表示模型输出、确定性 renderer 输出或分层生成结果没有通过 cypher-generator-agent 门禁，例如 `no_cypher_found`、`wrapped_in_markdown`、`contains_explanation`、`multiple_statements`、`unbalanced_brackets`、`unclosed_string`、`write_operation`、`unsupported_call`、`unsupported_start_clause`、`unauthorized_schema_reference`、`logical_plan_mismatch`、`semantic_match_rejected`。testing-agent 必须把它转成一次正式失败 attempt，进入 testing 流程评分。

生成失败 attempt 的评分边界：

1. testing-agent 为本轮分配 `attempt_no`，并保存 generation evidence。
2. testing-agent 从 `parsed_cypher` 派生 `candidate_query_text`：优先使用非空 `parsed_cypher`；如果没有可解析 Cypher，则使用空字符串。模型原始返回只保存在 `input_prompt_snapshot` 的对应 LLM 调用记录中，用于运行中心回放，不参与 grammar 输入。
...
6. 最终 `verdict = fail`，testing-agent 生成 IssueTicket，并把生成失败原因、候选 Cypher、`cga_trace_v2` 和 secondary signals 一起写入问题单。
...
| `generation_evidence` | cypher-generator-agent 生成过程证据。来源是 cypher-generator-agent 提交的 `GeneratedCypherSubmissionRequest` 或 `CgaGenerationNonSuccessReport`，testing-agent 只负责保存、关联 attempt，并写入问题单 |

### 1.6 `GenerationEvidence`

`GenerationEvidence` 表示 cypher-generator-agent 生成过程在问题单中的证据快照。

它的来源是 cypher-generator-agent 提交给 testing-agent 的 `GeneratedCypherSubmissionRequest` 或 `CgaGenerationNonSuccessReport`。testing-agent 不重新生成这些字段，只在接收时保存，并在生成 IssueTicket 时复制到 `generation_evidence`。

它包含：

| 字段 | 含义 |
| --- | --- |
| `generation_run_id` | cypher-generator-agent 本次生成运行 ID，用于把问题单与生成链路日志、生成证据快照串联起来 |
| `attempt_no` | testing-agent 记录的尝试序号。它不是 cypher-generator-agent 提交的字段，而是 testing-agent 接收 submission 后按同一 `id` 的历史记录分配 |
| `input_prompt_snapshot` | 生成证据快照，当前主契约为 `cga_trace_v2` |
| `generation_status` | `generated` 或 `generation_failed`；`clarification_required` 和 `service_failed` 只存在于非成功输出落盘记录，不进入 IssueTicket |
...
testing-agent 的评测从“同一 `id` 下 golden 与可评分生成结果都到齐”开始。可评分生成结果包括成功的 `GeneratedCypherSubmissionRequest`，也包括 `generation_status = generation_failed` 的 `CgaGenerationNonSuccessReport`。`clarification_required` 和 `service_failed` 只落盘，不进入正式评测。
...
| 步骤 | 输入 | 输出 | 意义 |
| --- | --- | --- | --- |
| 配对 | `QAGoldenRequest`、`GeneratedCypherSubmissionRequest` 或可评分 `CgaGenerationNonSuccessReport` | ready pair | 确保评测同时拥有标准答案和生成结果；澄清请求和工程失败只落盘，不配对评测 |
...
请求体结构与字段含义见第 1.3 节 `GeneratedCypherSubmissionRequest`。

testing-agent 接收 submission 后立即返回接收回执；字段定义见第 1.3 节 `GeneratedCypherSubmissionRequest`。如果同一 `id` 的 golden 已经存在，testing-agent 会在回执返回后，于内部链路继续执行评测，而不是让 cypher-generator-agent 同步等待完整评测结束。
...
请求体结构与字段含义见第 1.3.1 节 `CgaGenerationNonSuccessReport`。

接收规则：

1. 如果 `generation_status = service_failed`，testing-agent 只保存生成阶段工程失败，不分配正式 `attempt_no`，不进入评测主链路。
2. 如果 `generation_status = clarification_required`，testing-agent 只保存澄清报告，不分配正式 `attempt_no`，不进入评测主链路。
3. 如果 `generation_status = generation_failed`，testing-agent 必须保存 generation evidence，并在同一 `id` 的 golden 已存在时进入评测主链路；如果 golden 尚未到达，则等待 golden 到达后再评测。
4. 对 generation failed，testing-agent 会为本轮分配 `attempt_no`，并派生 `candidate_query_text` 供 grammar、GLEU 和 similarity 使用。
...
请求体为 `GeneratedCypherSubmissionRequest`，字段定义见第 1.3 节。

响应字段定义与第 1.3 节中的接收回执一致。它是 cypher-generator-agent 的提交确认，不是 qa-agent 的业务响应。
...
| `generation_failures/{id}__{generation_run_id}.json` | CGA 非成功输出报告；`generation_failed`、`clarification_required`、`service_failed` 的正式落点在这里 |
